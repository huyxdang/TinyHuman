...
async def run_simulation():
    ...
    for cluster in CLUSTERS:
        ...
        before_stances = {}
        for p in members:
            sr = search_map.get(p["id"], {})
            if sr.get("product_found"):
                before_stances[p["id"]] = "interested"
            elif sr.get("top_results"):
                before_stances[p["id"]] = "competitor"
            else:
                before_stances[p["id"]] = "neutral"
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            for p in members:
                sr = search_map.get(p["id"], {})
                search_summary = _build_search_summary(sr)

                system_msg = (
                    f"You are {p.get('name', 'Unknown')}, a {p.get('age', '?')} year old "
                    f"{p.get('job_title', '?')} from {p.get('province', '?')}.\n"
                    f"You searched for \"{sr.get('query', '?')}\" and found: {search_summary}\n"
        ...
        for p in members:
            vote_conversations.append([
                {"role": "system", "content": (
                    f"You are {p.get('name', 'Unknown')}. You just had a group discussion about {PRODUCT['name']}.\n"
        ...
        for p, raw_vote in zip(members, vote_outputs):
            choice, reason = _parse_vote(raw_vote, PRODUCT["name"], PRODUCT.get("competitors", []))
            ...
            cluster_decisions.append({
                "persona_id": p["id"],
                "name": p.get("name", "Unknown"),
                "before": before_stances.get(p["id"], "neutral"),
                "after": choice,
                "reason": reason,
            })
...
def _build_search_summary(sr: dict) -> str:
    if not sr:
        return "nothing — you didn't search"
    if sr.get("product_found"):
        return f"{PRODUCT['name']} appeared at rank {sr.get('product_rank', '?')}"
    results = sr.get("top_results", [])
    if results:
        top = [r.get("title", "?")[:40] for r in results[:3]]
        return "you found: " + ", ".join(top) + f" (not {PRODUCT['name']})"
...
def main():
    ...
    global PRODUCT, PERSONAS, CLUSTERS, SEARCH_RESULTS
    ...
    with open(args.search_results) as f:
        results = json.load(f)
        SEARCH_RESULTS = {r.get("persona_id"): r for r in results}
